...
class Media {
  ...
  onResize({
    ...
    this.plane.scale.x =
      (this.viewport.width * (1920 * this.scale)) / this.screen.width;
...
class App {
  ...
  createMedias(
    ...
    this.galleryCount = galleryItems.length;
  ...
  private emitActive() {
    if (!this.medias.length || !this.onActiveIndexChange) return;
    const w = this.medias[0]?.width || 1;
    const raw = Math.round(Math.abs(this.scroll.target) / w);
    const idx = ((raw % this.galleryCount) + this.galleryCount) % this.galleryCount;
  ...
  onWheel(e: Event) {
    ...
    // FIX scroll hijack: vertical wheel should scroll the page, not the gallery.
    // Only hijack when horizontal dominates (trackpad swipe) or Shift+wheel.
    const dx = wheelEvent.deltaX ?? 0;
    const dy = wheelEvent.deltaY ?? 0;
    const absX = Math.abs(dx);
    const absY = Math.abs(dy);
    const isShiftScroll = wheelEvent.shiftKey && dy !== 0 && absX === 0;
    const isHorizontal = absX > absY;

    // Vertical dominant without Shift -> let page scroll normally
    if (!isHorizontal && !isShiftScroll) return;

    const delta = isHorizontal ? dx : dy;
    const fallback =
      (wheelEvent as any).wheelDelta ?? (wheelEvent as any).detail ?? 0;
    const effectiveDelta = delta !== 0 ? delta : fallback;
    if (effectiveDelta === 0) return;

    this.scroll.target +=
      (effectiveDelta > 0 ? this.scrollSpeed : -this.scrollSpeed) * 0.2;
    // prevent page scroll only when we actually hijack horizontal/shift
  ...
  addEventListeners() {
    ...
    // Scope all interaction to the gallery container — not window.
    // This prevents dragging on Experience or empty space from moving the gallery.
    this.container.addEventListener("mousewheel", this.boundOnWheel, { passive: false } as any);
    ...
    // move/up stay on window so drag continues even if cursor leaves the gallery,
    // but onTouchDown only sets isDown when the initial target was inside container,
    // so empty-space drags never activate isDown.
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then((resolvedFont) => {
      ...
      app = new App(containerRef.current, {
        ...
        scrollSpeed,
        scrollEase,
        onActiveIndexChange: (idx: number) => callbackRef.current?.(idx),
      });
    });
    return () => {
      isMounted = false;
